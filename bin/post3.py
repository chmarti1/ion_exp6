...
for arg,label,style in zip(sys.argv[1::2], sys.argv[2::2], style_array):
    ...
    for this in os.listdir(datadir):
        if this.endswith(arg):
            if sourcedir:
                raise Exception(f'POST3.PY: found multiple data entries that end with {arg}')
    ...
    if not os.path.isfile(post1):
        raise Exception(f'POST3.PY: Failed to find post 1 results. Did you forget to run post1.py?\n Missing file ==> {post1}')
